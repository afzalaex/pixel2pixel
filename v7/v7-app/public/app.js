...
function currentLayout() {
  const seedPart =
    shuffleReady && snapshot && typeof snapshot.seedHash === "string"
      ? snapshot.seedHash
      : "natural";
  ...
  if (shuffleReady && snapshot && snapshot.seedHash) {
    const positions = buildShufflePositions(snapshot.seedHash);
...
function baseRoundStatus() {
  if (terminal) {
    if (shuffleReady) {
      return "Terminal snapshot locked. Auction finalized. Deterministic shuffle active.";
    }
    return "Terminal snapshot locked. Seeding frozen. Awaiting auction finalization.";
  }
...
function updateMetrics() {
  ...
  metricsEl.innerText = `minted: ${totalSupply} / ${MAX_NODES} | seeding: ${activeCount} / ${MAX_NODES} | gameActive: ${gameActive ? "true" : "false"}`;

  if (terminal && snapshot) {
    walletsEl.innerText = `snapshot -> block ${snapshot.blockNumber} | ts ${snapshot.timestamp} | seed ${snapshot.seedHash}`;
    return;
  }
...
function updateOwnerStatus() {
  ...
  if (ownedNode === 0) {
    ...
    if (terminal || gameActive || totalSupply >= MAX_NODES) {
...
async function applyRoundPayload(payload) {
  ...
  const nextGameActive = Boolean(payload.gameActive);
  const nextTerminal = Boolean(payload.terminal);
  const nextSnapshot = normalizeSnapshot(payload.snapshot);
  const nextShuffleReady = Boolean(payload.shuffleReady && nextSnapshot);
...
async function applySimulationState(mintedInput, seededInput) {
  ...
  const payload = {
    ...
    totalSupply: minted,
    gameActive: minted === MAX_NODES,
    terminal: minted === MAX_NODES && seeded === MAX_NODES,
    snapshot:
      minted === MAX_NODES && seeded === MAX_NODES
        ? buildSimSnapshot(minted, seeded)
        : null,
    shuffleReady: false
  };
